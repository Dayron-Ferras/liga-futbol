...
public class CrearCentrocampistaFrame extends JDialog {
    ...
    public CrearCentrocampistaFrame(ObtenerServices service) {
        super((Frame) null, "Crear Centrocampista", true);
        ...
        // Panel principal: fondo blanco translúcido, bordes redondeados
        ...
        gbc.gridx = 1;
        tfPasesClave = new JTextField(); tfPasesClave.setFont(textFont); tfPasesClave.setPreferredSize(new Dimension(200, 36));
        bgPanel.add(tfPasesClave, gbc); row++;

        gbc.gridx = 0; gbc.gridy = row;
        bgPanel.add(new JLabel("Recuperaciones:") {{ setFont(labelFont); }}, gbc);
        gbc.gridx = 1;
        tfIntercepciones = new JTextField(); tfIntercepciones.setFont(textFont); tfIntercepciones.setPreferredSize(new Dimension(200, 36));
        bgPanel.add(tfIntercepciones, gbc); row++;

        // Panel de botones (horizontal, bien alineados)
        ...
        btnCrear.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (
                    ...
                    JOptionPane.showMessageDialog(CrearCentrocampistaFrame.this, "¡Debe completar todos los campos!", "Campos incompletos", JOptionPane.WARNING_MESSAGE);
                    return;
                }
                try {
                    Centrocampista c = new Centrocampista();
                    c.setIdFutbolista(tfIdFutbolista.getText());
                    c.setPasesCompletos(Integer.parseInt(tfPasesClave.getText()));
                    c.setIntercepciones(Integer.parseInt(tfIntercepciones.getText()));
                    ...
                    o.createCentrocampistaCompleto(c, nombre, numero, edad, idEquipo, partidosJugados, goles, asistencias, promedioGoles);
                    dispose();
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(CrearCentrocampistaFrame.this, "Verifique que los campos numéricos tengan valores válidos.", "Error de formato", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
    }

    // Botón presionable y hover personalizado, con colores configurables
