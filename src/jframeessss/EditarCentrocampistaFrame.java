...
public class EditarCentrocampistaFrame extends JDialog {
    private JTextField tfIdFutbolista, tfNombre, tfNumero, tfEdad, tfIdEquipo, tfPartidos, tfGoles, tfAsistencias, tfPromedioGoles, tfPasesClave, tfIntercepciones;
    private HoverPressButton btnGuardar, btnCerrarRojo;

    public EditarCentrocampistaFrame(String idFutbolista) {
        super((Frame) null, "Editar Centrocampista", true);
        ...
        CentrocampistaService centrocampistaService = new CentrocampistaService();
        Centrocampista centrocampista = centrocampistaService.getCentrocampistaById(idFutbolista);
        ...
        gbc.gridx = 0; gbc.gridy = row;
        bgPanel.add(new JLabel("Pases Clave:") {{ setFont(labelFont); }}, gbc);
        gbc.gridx = 1;
        tfPasesClave = new JTextField(Integer.toString(centrocampista.getPasesCompletos()));
        tfPasesClave.setFont(textFont);
        tfPasesClave.setPreferredSize(new Dimension(200, 36));
        bgPanel.add(tfPasesClave, gbc); row++;

        gbc.gridx = 0; gbc.gridy = row;
        bgPanel.add(new JLabel("Intercepciones:") {{ setFont(labelFont); }}, gbc);
        gbc.gridx = 1;
        tfIntercepciones = new JTextField(Integer.toString(centrocampista.getIntercepciones()));
        tfIntercepciones.setFont(textFont);
        tfIntercepciones.setPreferredSize(new Dimension(200, 36));
        bgPanel.add(tfIntercepciones, gbc); row++;
        ...
        btnGuardar.addActionListener((ActionEvent evt) -> {
            if (
                ...
                tfPasesClave.getText().trim().isEmpty() ||
                tfIntercepciones.getText().trim().isEmpty()
            ) {
                JOptionPane.showMessageDialog(EditarCentrocampistaFrame.this, "¡Debe completar todos los campos!", "Campos incompletos", JOptionPane.WARNING_MESSAGE);
                return;
            } else {
                try {
                    centrocampistaService.eliminarCentrocampista(idFutbolista);
                    Centrocampista nuevoCentrocampista = new Centrocampista();
                    nuevoCentrocampista.setIdFutbolista(tfIdFutbolista.getText());
                    nuevoCentrocampista.setPasesCompletos(Integer.parseInt(tfPasesClave.getText()));
                    nuevoCentrocampista.setIntercepciones(Integer.parseInt(tfIntercepciones.getText()));
                    ...
                    CentrocampistaService o = new CentrocampistaService();
                    o.createCentrocampistaCompleto(nuevoCentrocampista, nombre, numero, edad, idEquipo, partidosJugados, goles, asistencias, promedioGoles);
                    dispose();
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(EditarCentrocampistaFrame.this, "Verifique que los campos numéricos tengan valores válidos.", "Error de formato", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
    }
